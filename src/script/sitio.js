...
const mostrarTodo = ()=>{
    ...
    <tr class="odd:bg-white odd:dark:bg-gray-900 even:bg-gray-50 even:dark:bg-gray-800 border-b dark:border-gray-700">
        <td class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">
            <img class="w-20 h-20" src="${element.linkImagen}" alt="imagen">
        </td>
        <td class="px-6 py-4">${element.codigo}</td>
        <td class="px-6 py-4">${element.nombre}</td>
        <td class="px-6 py-4">$${element.precio}</td>
        <td class="px-6 py-4">
            <button class="agregar-carrito bg-blue-500 text-white py-2 px-4 rounded hover:bg-blue-700" type="button" data-index="${index}" id="agregar">
                Agregar al carrito
            </button>
        </td>
    </tr>
